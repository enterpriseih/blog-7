...
/*
 * Modified by ZhongDuying for show format.
 *   2014.07.01
 */
...
macro Review_Restore_Link()
{
	...
	while(True)
	{
		...
		lnNumber = strmid(str, 13, strlen(str))		/* 13 = strlen("Line       : ") */
...
macro Review_Add_Comment()
{
	...
	if(sTemp == "Defect缺陷")
	{
		...
		/* get defect type */
		promote = "Categories : F,f(功能); I,i(接口); A,a(赋值); C,c(注释); L,l(日志); M,m(算法); T,t(时序/顺序缺陷); O,o(其它)"
		sTemp = ask(promote);
		sTemp = toupper(sTemp[0]);
		while( sTemp != "F" && sTemp != "I" 
			&& sTemp != "A" && sTemp != "C" && sTemp != "L"
			&& sTemp != "M" && sTemp != "T" && sTemp != "O")
		{
			sTemp = ask(cat("Please input again! ", promote));
			sTemp = toupper(sTemp[0]);
		}
		
		if ( sTemp == "F" ) sTemp = "Function功能";
		else if( sTemp == "I" ) sTemp = "Interface接口";
		else if ( sTemp == "A" ) sTemp = "Assignment赋值";
		else if ( sTemp == "C" ) sTemp = "Comment注释";  
		else if ( sTemp == "L" ) sTemp = "Log日志";
		else if ( sTemp == "M" ) sTemp = "algorithM算法";
...
macro Review_Output_123()
{
	...
	rvTitle = "it1=\"FileName :\";it2=\"评审人员\t\";it3=\"函数\t\";it4=\"问题类型\t\";it5=\"严重级别\t\";it6=\"缺陷来源\t\";it7=\"缺陷类型\t\";it8=\"描述\t\";it9=\"状态\t\";it10=\"作者\t\";it11=\"修改说明\t\""
	...
	sOutput = cat(sOutput,rvTitle.it11)
	...
	while (True)
	{
		...
		tpsel = SearchInBuf(hbuf, "^Symbol\\s+:\\s+", ln, 0, 1, 1, 0)
		if(tpsel)
		{
			tpln = tpsel.lnFirst
			tpcol = tpsel.ichLim
